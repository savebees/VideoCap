"""Strict fixed-window -> event-grounded caption production recipe."""
...
from dense_video_annotator.io import atomic_write_json, atomic_write_jsonl
from dense_video_annotator.protocols import DenseCaptionOutcome, DenseCaptionRecipe, DenseCaptionSample
...
def _window_caption(window: ProcessingWindow, raw: Mapping[str, Any]) -> WindowCaption:
    ...
    normalized = WindowCaption(window.window_id, captions, tuple(evidence))
...
@RECIPES.register("structured-grounding")
class StructuredGroundingRecipe(DenseCaptionRecipe):
    name = "structured-grounding"
    version = "0.1"
    ...
    def _adapters(self, cfg: Mapping[str, Any]) -> StructuredAdapterBundle:
        ...
        if isinstance(factory_path, str) and factory_path.strip():
            ...
            if not isinstance(bundle, StructuredAdapterBundle):
                raise TypeError("structured-grounding adapter_factory must return StructuredAdapterBundle")
            return bundle
        paths = cfg.get("adapters")
        if not isinstance(paths, Mapping):
            raise RuntimeError("structured-grounding requires recipe_config.adapters")
        fields = {
        ...
        for field, config_key in fields.items():
            ...
            if not isinstance(value, str) or not value.strip():
                raise RuntimeError(f"structured-grounding stage '{config_key}' has no adapter")
    ...
    def produce(self, samples: Sequence[DenseCaptionSample], work_dir: Path, config: Mapping[str, Any]) -> Sequence[DenseCaptionOutcome]:
        ...
        if not isinstance(final_output_name, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*\.jsonl", final_output_name):
            raise ValueError("recipe_config.final_output_name must be a safe .jsonl filename")
        if isinstance(max_duration, bool) or not isinstance(max_duration, int):
            raise ValueError("recipe_config.max_duration_ms must be an integer")
        if isinstance(overlap, bool) or not isinstance(overlap, int):
            raise ValueError("recipe_config.overlap_ms must be an integer")
        if isinstance(evidence_count, bool) or not isinstance(evidence_count, int):
            raise ValueError("recipe_config.evidence_frame_count must be an integer")
    ...
    def _produce_one(self, sample: DenseCaptionSample, directory: Path, adapters: StructuredAdapterBundle, max_duration: int, overlap: int, evidence_count: int) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        structured_result = {
            "schema_version": "structured-grounding/v0.1",
        ...
        validate_document(structured_result, "structured_grounding")
