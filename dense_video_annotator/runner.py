...
from dense_video_annotator.contracts import ComponentRef, DatasetRef, RunManifest
from dense_video_annotator.io import atomic_write_csv, atomic_write_json, atomic_write_jsonl
from dense_video_annotator.protocols import (
    DenseCaptionDataset,
    DenseCaptionMetric,
    DenseCaptionOutcome,
)
from dense_video_annotator.registry import METRICS, RECIPES, TASKS
...
def _new_run_id(task_name: str, recipe_name: str) -> str:
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    return f"{task_name}__{recipe_name}__{timestamp}__{uuid.uuid4().hex[:8]}"
...
def run_dense_caption(
    ...
    *,
    ...
    recipe_name: str = "cosmos-curator",
    ...
    recipe = RECIPES.create(recipe_name)
    ...
    resolved_run_id = run_id or _new_run_id(task.name, recipe.name)
    ...
    manifest = RunManifest(
        ...
        recipe=ComponentRef(recipe.name, recipe.version),
    ...
    work_dir = run_dir / "recipe_work"
    try:
        raw_outcomes = recipe.produce(samples, work_dir, run_config)
        outcomes: dict[str, DenseCaptionOutcome] = {}
        for outcome in raw_outcomes:
            if not isinstance(outcome, DenseCaptionOutcome):
                raise TypeError("recipe outputs must be DenseCaptionOutcome instances")
            if outcome.video_id not in sample_by_id:
                raise ValueError(f"recipe returned unknown video_id: {outcome.video_id}")
            if outcome.video_id in outcomes:
                raise ValueError(f"recipe returned duplicate video_id: {outcome.video_id}")
    ...
    for sample in samples:
        ...
        if outcome is None:
            outcome = DenseCaptionOutcome(
                video_id=sample.video_id,
                error_type="MissingRecipeOutput",
                message="recipe returned no outcome for this video",
            )
        ...
        if outcome.artifact is not None:
            try:
                validate_document(outcome.artifact, "structured_grounding")
